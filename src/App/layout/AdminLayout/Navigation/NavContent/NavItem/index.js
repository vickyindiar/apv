...
class NavItem extends Component {
    ...
    render() {
        ...
        if (this.props.layout === 'horizontal') {
            mainContent = (
                <li id={this.props.item.id} onClick={this.props.onItemLeave}>{subContent}</li>
            );
        } else {
            if (this.props.windowWidth < 992) {
                mainContent = (
                    <li id={this.props.item.id}  className={this.props.item.classes} onClick={this.props.onItemClick}>{subContent}</li>
                );
            } else {
                mainContent = (
                    <li id={this.props.item.id}  className={this.props.item.classes}>{subContent}</li>
                );
            }
        }
